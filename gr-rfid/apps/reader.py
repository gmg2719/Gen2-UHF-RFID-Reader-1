...
class reader_top_block(gr.top_block):
  ...
  def __init__(self):
    ...
    #self.file_sink_source         = blocks.file_sink(gr.sizeof_gr_complex*1, "../misc/data/raw_source", False)
    ...
    if (DEBUG == False) : # Real Time Execution
      ...
      #self.file_source               = blocks.file_source(gr.sizeof_gr_complex*1, "../misc/data/temp",False)   ## instead of uhd.usrp_source
      self.file_source               = blocks.file_source(gr.sizeof_gr_complex*1, "50mw/"+sys.argv[1]+"/matched_filter",False)   ## instead of uhd.usrp_source
